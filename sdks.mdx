...
The response is an Anthropic `message` object. Output lives in `content` as `text`, `thinking`, and `tool_use` blocks; the served tier is authoritative from `usage.service_tier`:
