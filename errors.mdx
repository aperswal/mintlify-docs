...
| `400` | <span id="unsupported_tool" /> `unsupported_tool` | The `tools` array contains a built-in tool that makes the model emit non-text output (currently `image_generation`). FlexInference serves text output only, on every endpoint. Remove that tool entry; function tools and `web_search` stay supported. |
