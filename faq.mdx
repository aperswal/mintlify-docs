...
## What happens if FlexInference cannot find cheaper inference?

If the flex race cannot find cheaper inference within your start_within deadline, we escalate the request to a standard, default-tier request, so you still get your response. We only charge a commission when a flex request actually saves you money, so a request that finds nothing cheaper carries no fee.
...
## Does FlexInference change my model, prompt, or output?

No. We call the exact model you requested with your exact prompt, and pass the response back unchanged. The only thing we change is how the request is routed, which tier or provider serves that same model, to find it cheaper. We never swap models, rewrite prompts, or alter responses, and we never silently strip a parameter you set to force a request through.
