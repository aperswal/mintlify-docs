...
## What FlexInference changes

FlexInference changes the tier, and nothing about the meaning of your request or its result. It never rewrites a provider's status or message. The one thing it normalizes is the error envelope: an error is returned in the shape of the endpoint you called (OpenAI on responses and chat, Anthropic on messages, Google on interactions), carrying the provider's status and message, so the SDK you are already using can parse it. See [Errors](/errors).
