...
FlexInference does not resell inference. You add your own provider key once in the dashboard - an **OpenAI** key (`sk-…`) for GPT models, a **Gemini** key (`AIza…`) for Gemini models, an **Anthropic** key (`sk-ant-…`) for Claude models, or any mix - and we use the matching key to call that provider on your behalf. The provider bills your account directly. We pick the key by the model: a `gemini-*` model uses your Gemini key, a `claude-*` model uses your Anthropic key, everything else uses your OpenAI key.
